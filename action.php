...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM categoria";
...
if(isset($_POST["getProduct"])){
	...
	$product_query = "SELECT * FROM producto LIMIT $start,$limit";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$pro_id    = $row['idproducto'];
			$pro_cat   = $row['idcategoria'];

			$pro_title = $row['descripcion'];
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>S/.$pro_price
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Agregar</button>
								</div>
							</div>
						</div>
			";
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		...
		$sql = "SELECT * FROM producto WHERE descripcion LIKE '%$keyword%'";
	}
...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT p.idproducto, p.descripcion, p.stock, p.precio, p.imagen,  c.id, c.qty FROM producto p, cart c WHERE p.idproducto=c.p_id AND c.user_id='$_SESSION[uid]'";
	}else{
		//When user is not logged in this query will execute
		$sql = "SELECT p.idproducto, p.descripcion, p.stock, p.precio, p.imagen,  c.id, c.qty FROM producto p, cart c WHERE p.idproducto=c.p_id AND c.ip_add='$ip_add' AND c.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				echo '
					<div class="row">
						...
						<div class="col-md-3">S/.'.$product_price.'</div>
					</div>';
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form id='checkout_form' method='post' action='login_form.php'>";
				$n=0;
				while ($row=mysqli_fetch_array($query)) {
					$n++;
					$product_id = $row["idproducto"];
					...
					echo
						'<div class="row">
								...
								<div class="col-md-2"><input type="text" class="form-control price" name="price" value="'.$product_price.'" readonly="readonly"></div>
								<div class="col-md-2"><input type="text" class="form-control total" value="'.$product_price.'" readonly="readonly"></div>
								<input type="hidden" name="add_pedidos" value="1"/>
							</div>';
				}
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Realizar Pedido" >
							</form>';
				}
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							echo
								'<input type="hidden" name="return" value="http://localhost/project1/payment_success.php"/>
									...
									<a href="#" id="btn_finish_order" class="btn btn-lg btn-success">Finalizar pedido</a>
								</form>';
				}
